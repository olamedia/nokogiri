...
class nokogiri implements IteratorAggregate
{
    ...
    public function getXpathSubquery($expression, $rel = false, $compile = true)
    {
        ...
        if (preg_match(self::regexp, $expression, $subs)) {
            ...
            $query = ($rel ? '/' : '//' ).
                    ...
                    (($c = count($brackets)) ? ($c>1 ? '[('. implode(') and (', $brackets) .')]' : '['. implode(' and ', $brackets) .']') :'');
    ...
    public function toHtml()
    {
        ...
        {
            $_tempDom = new \DOMDocument('1.0', 'UTF-8');

            if ($this->_dom instanceof \DOMNodeList)
            {
                foreach ($this->_dom as $domElement) {
                    $domNode = $_tempDom->importNode($domElement, true);
                    $_tempDom->appendChild($domNode);
                }
            }
            else
            {
                $domNode = $_tempDom->importNode($this->_dom, true);
                $_tempDom->appendChild($domNode);
            }

            return $_tempDom->saveHTML();
        }
